  ...
  return (
    <div>
      <section className=' relative top-16 sm:relative left-8 sm:top-4'>
          ...
          <p className='relative top-20 text-center sm:relative sm:text-center text-sm mt-4 sm:top-20  '>Simply browse through our extensive list of trusted doctors,<br />schedule your appointment hassle-free.</p>
      </section>
       <section id='Speciality' className=' overflow-x-scroll relative top-12 left-60 flex justify-center sm:flex sm:relative sm:-left-1 gap-5 sm:justify-center mt-20 text-center'>
            { specialityData.map((item , index)=>(
              <Link key={index} to={`/doctors/${item.speciality}`}>
                  <img className='sm:size-20 mt-0' src={item.image}   />
