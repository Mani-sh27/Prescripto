  ...
  return (
    <div>
      <section className=' relative top-16 sm:relative sm:top-4'>
       ...
       <section id='Speciality' className=' overflow-x-scroll relative top-12 left-60 flex justify-center sm:flex gap-5 sm:justify-center mt-30 text-center'>
            { specialityData.map((item , index)=>(
              <Link key={index} to={`/doctors/${item.speciality}`}>
                  <img className='sm:size-20 mt-8' src={item.image}   />
                  <p className=' ms-56 sm:text-sm mt-2'>{item.speciality}</p>
              </Link>
              ))}
       </section>
    </div>
