...
const Doctors = () => {
  return (
    <div>
      <p className='text-center mt-30 text-5xl sm:ms-0 ms-72 font-semibold '>Top Doctors</p>
      ...
      <div className='w-[850px] gap-8 grid grid-cols-2  sm:gap-5 sm:grid sm:w-[1240px] mt-10 relative left-32 sm:grid-cols-5'>
        {
          doctors.slice(0,10).map((item,index)=>(
              <div className='rounded-2xl  transition-all hover:-translate-y-2 cursor-pointer border-blue-200 shadow-sm '>
                <img className='bg-blue-50 rounded-t-2xl' src={item.image}  /> 
                <div>
                  <p className='w-2 h-2 rounded-full bg-green-500 relative top-7.5 left-3'></p><p className='my-3 ms-7 text-green-500'>Available</p>
      ...
      <button className='sm:flex  bg-gray-200 my-16 py-4 px-16 ms-160 cursor-pointer rounded-full'>More</button>
    </div>
  )
